...
double Kd = 1.478;
*/
double Kp = 0.01; //0.0029; //0.0001;
double Ki = 0.4; //1.7394e-04; //0.3
double Kd = 0.004; //0.0166;//0.007
double Tm = 0.05;
...
void subscription_callback(const void *msgin) {
  ...
  sp = msg_in->data;                                        //Volvemos pwm_cycle y lo ponemos en una variable global
...
void timer_callback_1(rcl_timer_t * timer, int64_t last_call_time) {
  ...
  //cv = Kp * error + (Kd / Tm) * (error - error1) + Ki * Tm * (error - error1); //Ecuacion de proporcionada por Manchester
  cv = Kp*error + error*Kd / Tm + error*Ki* Tm;
  cv1 = cv;
  error2 = error1;
  error1 = error;
